...
public class DeliveryApp extends App {
    ...
    String name;
    List<IDeliverable> registeredCafe = new ArrayList<>();
    IDeliverable cafe;
    Customer user;
    ...
    public void register(IDeliverable cafe) {registeredCafe.add(cafe);}
    public void printRegisteredCafe() {
        System.out.println("현재 배달 가능한 카페 리스트:");
        for(IDeliverable cafe : registeredCafe) {
        ...
        for (Drink drink : ((Cafe) this.cafe).getMenu()){
            if(drink.getName().equals(menuName)) {
                ...
                if(this.user.creditCard.payLater(drink.price)) {
                    ...
                    Order tempOrder = new Order((IDeliverable)tempCafe, this.user,
